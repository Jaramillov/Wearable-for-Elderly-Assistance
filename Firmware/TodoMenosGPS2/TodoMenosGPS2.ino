...
const unsigned long ALERT_DURATION = 5000; // 5 segundos en rojo después de alerta
...
const String NUMERO_EMERGENCIA = "+5731034xxxx";
...
void loop() {
  ...
  if (now - last_mpu_check >= MPU_INTERVAL) {
    ...
    // Determinar estado según el score de caída
    if (fall_score >= FALL_ALERT_THRESHOLD) {
      ...
      // Si hay una alerta activa, verificar si ya pasó el tiempo
      if (alertaActiva && (now - alertStartTime > ALERT_DURATION)) {
        alertaActiva = false;
      }
  ...
  // CAMBIO CRÍTICO: Usar portMAX_DELAY para esperar lectura completa
  i2s_read(I2S_PORT, 
           (void *)bufferI2S, 
           EI_BUFFER_SIZE * sizeof(int16_t), 
           &bytes_read, 
           portMAX_DELAY);

  // Verificar que se leyó la cantidad correcta de bytes
    ...
    if (err == EI_IMPULSE_OK) {
      ...
      for (size_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
        ...
        if (label == "ayuda") {
          ...
          noise = result.classification[i].value;
        } else if (label == "unknown") {
          unknown = result.classification[i].value;
        }
      }

      float otros = noise + unknown;

      // Imprimir valores de clasificación de voz
      ...
      Serial.print("[ERROR] Clasificador fallo con codigo: ");
      Serial.println(err);
    }
  }

  // Verificar si la alerta activa debe apagarse por tiempo
  if (alertaActiva && (now - alertStartTime > ALERT_DURATION)) {
    alertaActiva = false;
    Serial.println("[ALERTA FINALIZADA - Retornando a estado normal]");
  }
